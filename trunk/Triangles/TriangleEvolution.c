...
int TriangleChanger(double *Triangles, double *Fit, int *TriangleCount, double ***matrix, double ***image, int flag)
{
  ...
  for(j=0;j<*TriangleCount;j++){
    ...
    do{
      escaped=0;
      steps++;
      if(steps>(flag?MaximumSteps:MaximumSteps)){escaped=1;break;}
...
int main (int argc, char *argv[]){
  ...
  double ***image, ***AuxMatrix, ***AuxMatrix2, init; 
  ...
  fscanf(input, " %lf ", &image[0][0][0]); //get rid of resolution
  fscanf(input, " %lf ", &image[0][0][0]); //get rid of maxcolor
